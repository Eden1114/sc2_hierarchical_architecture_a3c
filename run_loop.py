...
def run_loop(agents, env, max_frames,
             ind_thread):  # agents是列表，里面有一个agent（A3CAgent对象），env是SC2Env对象经过处理后的变量，max_frames是回合内最多进行的step数
    ...
    try:
        while True:  # 底下发生的是一个回合内的过程
            ...
            while True:  # 底下发生的是回合内一步的过程
                ...
                action, call_step_low, act_id, macro_type, coord_type = action_micro(ind_thread, dir_high, ind_todo)
                ...
                is_done = (num_frames >= max_frames) or timesteps[
                    0].last()  # timesteps[0]是timesteps的第一个变量step_type（状态类型），last()为True即到了末状态
                yield [last_timesteps[0], action[0],
                       timesteps[0]], is_done, num_frames, call_step_low, macro_type, coord_type
