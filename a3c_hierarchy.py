...
def low_reward(state, next_state, location, macro_action_id):
    ...
    if macro_action_id == 1:  # 建造补给站的距离
        distance = location[0] - supply[0] + location[1] - supply[1]
        if 1 < distance <= 5:
            reward += 500
        elif distance <= 1:
            reward += 0
    ...
    if macro_action_id == 2:  # 建造兵营的距离
        distance = location[0] - barrack[0] + location[1] - barrack[1]
...
class A3C:  # TODO 注意：传参名称为macro-micro，网络占位符名称为high-low
    def __init__(self, sess, reuse):
        ...
        with tf.variable_scope('agent') and tf.device('/gpu:0'):
            ...
            low_train_function = tf.train.RMSPropOptimizer(self.lr, decay=0.99, epsilon=1e-10)
            ...
            high_train_function = tf.train.RMSPropOptimizer(self.lr, decay=0.99, epsilon=1e-10)
    ...
    def choose_high_action(self, state):
        ...
        if np.random.rand() < 0.05:
            macro_action_id = np.random.randint(0, 6)
...
def run(agent, max_epoch, map_name):
    ...
    for epoch in range(max_epoch):
        ...
        micro_reward, macro_reward = 0, 0  # 避免警告，实际无用
        while True:  # 每一个step执行一个动作
            ...
            if len(micro_buffer) > 100:  # TODO buffer学习的判断具体设置（num_frames >= max_frames）
                agent.update_low(micro_buffer)  # TODO 是否加入lr衰减
                micro_buffer = []

            if len(macro_buffer) > 20:  # TODO buffer学习的判断具体设置（num_frames >= max_frames）
                agent.update_high(macro_buffer)  # TODO 是否加入lr衰减
                macro_buffer = []

            if next_state.last():
                try:  # TODO 捕获buffer[-1][-1]取值失败，具体原因不明
                    macro_error = macro_buffer[-1][-1]
                    micro_error = micro_buffer[-1][-1]
                except IndexError:
                    print('error buffer lens: ', len(macro_buffer), len(micro_buffer))
                else:
        ...
        if epoch % 1 == 0:
            print("epoch reward: ", epoch, macro_reward + micro_reward)
