...
# DHN add:
from ppo_network import build_high_net
from ppo_network import build_low_net
...
class PPOAgent(object):
    ...
    def __init__(self, training, msize, ssize, name='PPO/PPOAgent'):
        '''[summary]
        参数为
        是否为训练模式（bool值）
        minimap分辨率（64，整型）
    ...
    def build_model(self, reuse, dev):
        with tf.variable_scope(self.name) and tf.device(dev):
            ...
            # Build networks
            # DHN add:
            self.dir_high, self.value_high, self.a_params_high, self.c_params_high = build_high_net("pi",
                                                                                                    self.minimap,
                                                                                                    self.screen,
                                                                                                    self.info_high,
                                                                                                    num_macro_action)
            ...
            self.dir_high_old, _, self.a_params_high_old, _ = build_high_net("oldpi",
                                                                                                    ...
                                                                                                    self.info_high,
                                                                                                    num_macro_action)
            with tf.variable_scope('update_high_oldpi'):
                self.update_high_oldpi_op = [oldp.assign(p) for p, oldp in zip(self.a_params_high, self.a_params_high_old)]
    ...
    def update_high(self, ind_thread, rbs, dhs, disc, lr_a, lr_c, cter):
        ...
        # dhs(dir_high_buffers) 是指令序号的集合。比如一共有5个宏动作，则dhs形如[[0], [4], [1], [2], [3], [4], [2], [0], ......]
        ...
        feed_s = {self.minimap: minimaps,
                  ...
                  self.learning_rate_a_high: lr_a,
                  self.learning_rate_c_high: lr_c}
        summary = self.sess.run(self.summary_op_high, feed_dict=feed_s)
        self.summary_writer.add_summary(summary, cter)
        print("summarize high ends")
